...
# RK-4 method
def rk4(t0, y0, max_iterations=10000, max_t=1000, iter_output=1000):
    
    def f(t_i,current_state, progress_bar, sigma):
         return get_rhs(t_i, current_state, \
            R_max, N_r, r_is_logarithmic, sigma, progress_bar, [t_ini, dt])
    ...
    with tqdm(total=1, unit=" ") as progress_bar:
         for i in range(max_iterations):
            ...
            k1 = dt * f(t0, y0, progress_bar, sigma)
            k2 = dt * f((t0+dt/2), (y0+k1/2),progress_bar, sigma)
            k3 = dt * f((t0+dt/2), (y0+k2/2),progress_bar, sigma)
            k4 = dt * f((t0+dt), (y0+k3), progress_bar, sigma)
            k = (k1+2*k2+2*k3+k4)/6
            yn = y0 + k
...
solution, t_sol = rk4(t_ini, initial_state, \
                       max_iterations=N_t, max_t=T, iter_output=N_t//8) 

print(solution.shape, t_sol.shape ) 
