...
def unpack_state(vars_vec, N_r) :
    ...
    # Misner-Sharp variables 
    U    = vars_vec[idx_U * domain_length : (idx_U + 1) * domain_length]
    R    = vars_vec[idx_R * domain_length : (idx_R + 1) * domain_length]
    M    = vars_vec[idx_M * domain_length : (idx_M + 1) * domain_length]
    rho  = vars_vec[idx_rho * domain_length : (idx_rho + 1) * domain_length]
    #A    = vars_vec[idx_U * domain_length : (idx_A + 1) * domain_length]]
    
    
    return U, R, M, rho

def pack_state(vars_vec, N_r, U, R, M, rho) :
    ...
    vars_vec[idx_R * domain_length : (idx_R + 1) * domain_length] = R
    vars_vec[idx_M * domain_length : (idx_M + 1) * domain_length] = M
    vars_vec[idx_rho * domain_length : (idx_rho + 1) * domain_length] = rho
