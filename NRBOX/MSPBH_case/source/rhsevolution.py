...
def get_rhs(t_i, current_state, params, sigma, progress_bar, time_state) :     
    ...
    if(r_is_logarithmic) : #take logarithmic derivatives
        ...
        # d2rhodr2   = get_d2fdx2(rho, oneoverdxsquared)
        ...
        # # first derivatives L
        # dUdr_L       = get_dfdx_advec_L(U, oneoverdx)
        # dRdr_L       = get_dfdx_advec_L(R, oneoverdx)
        # dMdr_L       = get_dfdx_advec_L(M, oneoverdx)
        # drhodr_L     = get_dfdx_advec_L(rho, oneoverdx)
        
        # # first derivatives R
        # dUdr_R       = get_dfdx_advec_R(U, oneoverdx)
        # dRdr_R       = get_dfdx_advec_R(R, oneoverdx)
        # dMdr_R       = get_dfdx_advec_R(M, oneoverdx)
        # drhodr_R     = get_dfdx_advec_R(rho, oneoverdx)
        
        # # Try correct rho and U  --> (Doesn't do anything) 
        # if False:
            # mask_r = (r>R_max/10) *   (r<R_max/2)
            # mask_Upos =  (d2Udr2>0) * mask_r
            # mask_Uneg =  (d2Udr2<0) * mask_r
            
            # #mask_Upos[:-len(U)//2] = 0
            # #mask_Uneg[:-len(U)//2] = 0
            
            # dUdr[mask_Upos] = dUdr_L[mask_Upos]
            # dUdr[mask_Uneg] = dUdr_R[mask_Uneg]
            
            # mask_Upos =  (d2rhodr2>0) * mask_r
            # mask_Uneg =  (d2rhodr2<0) * mask_r
            
            # #mask_Upos[:-len(U)//2] = 0
            # #mask_Uneg[:-len(U)//2] = 0
            
            # drhodr[mask_Upos] = drhodr_R[mask_Upos]
            # drhodr[mask_Uneg] = drhodr_L[mask_Uneg]
