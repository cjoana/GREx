...
def get_initial_state(R, N_r, r_is_logarithmic) :
    ...
    for ix in range(num_ghosts, N) :
        ...
        initial_state[ix + idx_a * N]   = em4chi * grr_here      # -1.0  
        initial_state[ix + idx_b * N]   = em4chi * gtt_over_r2   # -1.0
    ...
    # needed for lambdar
    hrr    = initial_state[idx_a * N : (idx_a + 1) * N] -1.0
    htt    = initial_state[idx_b * N : (idx_b + 1) * N] -1.0
    hpp    = initial_state[idx_b * N : (idx_b + 1) * N] -1.0
    ...
    for ix in range(num_ghosts, N-num_ghosts) :
        ...
        initial_state[ix + idx_Lambda * N]   = Delta_U[i_r]


    # Fill boundary cells for Lambda
    fill_outer_boundary_ivar(initial_state, dx, N, r_is_logarithmic, idx_Lambda)

    # overwrite inner cells using parity under r -> - r
    fill_inner_boundary_ivar(initial_state, dx, N, r_is_logarithmic, idx_Lambda)
