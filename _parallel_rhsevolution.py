...
def get_rhs(t_i, current_state, R, N_r, r_is_logarithmic, eta, progress_bar, time_state) :
    ...
    _rhs_u   = shared_memory.SharedMemory(create=True, size=np.zeros_like(u).nbytes)
    _rhs_v   = shared_memory.SharedMemory(create=True, size=np.zeros_like(v).nbytes)
    _rhs_phi = shared_memory.SharedMemory(create=True, size=np.zeros_like(phi).nbytes)
    _rhs_hrr = shared_memory.SharedMemory(create=True, size=np.zeros_like(hrr).nbytes)
    _rhs_htt = shared_memory.SharedMemory(create=True, size=np.zeros_like(htt).nbytes)
    _rhs_hpp = shared_memory.SharedMemory(create=True, size=np.zeros_like(hpp).nbytes)
    _rhs_K   = shared_memory.SharedMemory(create=True, size=np.zeros_like(K).nbytes)
    _rhs_arr = shared_memory.SharedMemory(create=True, size=np.zeros_like(hrr).nbytes)
    _rhs_att = shared_memory.SharedMemory(create=True, size=np.zeros_like(htt).nbytes)
    _rhs_app = shared_memory.SharedMemory(create=True, size=np.zeros_like(hpp)    .nbytes)
    _rhs_lambdar = shared_memory.SharedMemory(create=True, size=np.zeros_like(lambdar).nbytes)
    _rhs_shiftr  = shared_memory.SharedMemory(create=True, size=np.zeros_like(shiftr).nbytes)
    _rhs_br     = shared_memory.SharedMemory(create=True, size=np.zeros_like(br).nbytes)
    _rhs_lapse   = shared_memory.SharedMemory(create=True, size=np.zeros_like(lapse).nbytes)

    
    var = np.zeros_like(u)    
    rhs_u   =  np.ndarray( var.shape, var.dtype , buffer=_rhs_u.buf)
    rhs_v   =  np.ndarray(var.shape, var.dtype , buffer=_rhs_v.buf)
    rhs_phi =  np.ndarray(var.shape, var.dtype , buffer=_rhs_phi.buf)
    rhs_hrr =  np.ndarray(var.shape, var.dtype , buffer=_rhs_hrr.buf)
    rhs_htt =  np.ndarray(var.shape, var.dtype , buffer=_rhs_htt.buf)
    rhs_hpp =  np.ndarray(var.shape, var.dtype , buffer=_rhs_hpp.buf)
    rhs_K   =  np.ndarray(var.shape, var.dtype , buffer=_rhs_K.buf)
    rhs_arr =  np.ndarray(var.shape, var.dtype , buffer=_rhs_arr.buf)
    rhs_att =  np.ndarray(var.shape, var.dtype , buffer=_rhs_att.buf)
    rhs_app =  np.ndarray(var.shape, var.dtype , buffer=_rhs_app.buf)
    rhs_lambdar =  np.ndarray(var.shape, var.dtype , buffer=_rhs_lambdar.buf)
    rhs_shiftr  =  np.ndarray(var.shape, var.dtype ,  buffer=_rhs_shiftr.buf)
    rhs_br     =  np.ndarray(var.shape, var.dtype , buffer=_rhs_br.buf)
    rhs_lapse   =  np.ndarray(var.shape, var.dtype  , buffer=_rhs_lapse.buf) 
...
#        pool.map(task, range(num_ghosts, N-num_ghosts))
    #for ix in range(num_ghosts, N-num_ghosts) :
    #     task(ix)
...
#    with concurrent.futures.ProcessPoolExecutor() as executor:
        for ix in range(num_ghosts, N-num_ghosts) :
            executor.submit(task, ix)
#        #executor.map(task, range(num_ghosts, N-num_ghosts))
#        #values = {k: k for k in range(num_ghosts, N-num_ghosts)}
#        # Launch threads
#        #res = {k: executor.submit(task, v) for k, v in values.items()}
#        executor.shutdown(wait=True)

#    from multiprocessing import Process
#
#    for ix in range(num_ghosts, N-num_ghosts) :
#        p = Process(target=task, args=(ix,))
#        p.start()
#        p.join()
#
