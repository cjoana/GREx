...
def get_rhs(t_i, current_state, R, N_r, r_is_logarithmic, eta, progress_bar, time_state) :
    ...
    def parallel_rhs(ix): 
        ...
        rhs_u_ix, rhs_v_ix = get_matter_rhs(u[ix], v[ix], dudx[ix], d2udx2[ix], 
                                              r_gamma_UU, em4phi, dphidx[ix], 
                                              K[ix], lapse[ix], dlapsedx[ix], r_conformal_chris)

        # Get the bssn rhs - see bssnrhs.py
        rhs_phi_ix     = get_rhs_phi(lapse[ix], K[ix], bar_div_shift)
        
        rhs_h           = get_rhs_h(r_here, r_gamma_LL, lapse[ix], traceA, dshiftrdx[ix], shiftr[ix], 
                                    bar_div_shift, a)
        
        rhs_K_ix       = get_rhs_K(lapse[ix], K[ix], Asquared, em4phi, d2lapsedx2[ix], dlapsedx[ix], 
        ...
        rhs_lambdar_ix = get_rhs_lambdar(r_here, d2shiftrdx2[ix], dshiftrdx[ix], shiftr[ix], h, dhdr,
        ...
        rhs_br_ix     = 0.75 * rhs_lambdar_ix - eta * br[ix]
        rhs_shiftr_ix = br[ix]
        rhs_lapse_ix  = - 2.0 * lapse[ix] * K[ix]        
        ...
        if (shiftr[ix] > 0) :
            rhs_u_ix       += shiftr[ix] * dudx_advec_R[ix]
            rhs_v_ix       += shiftr[ix] * dvdx_advec_R[ix]
            rhs_phi_ix     += shiftr[ix] * dphidx_advec_R[ix]
            rhs_hrr_ix     = (rhs_h[i_r][i_r] + shiftr[ix] * dhrrdx_advec_R[ix] 
                               + 2.0 * hrr[ix] * dshiftrdx[ix])
            rhs_htt_ix     = (rhs_h[i_t][i_t] + shiftr[ix] * dhttdx_advec_R[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * h[i_t][i_t]) # additional advection terms from rescaling
            rhs_hpp_ix     = (rhs_h[i_p][i_p] + shiftr[ix] * dhppdx_advec_R[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * h[i_p][i_p]) # additional advection terms from rescaling
            rhs_K_ix       += shiftr[ix] * dKdx_advec_R[ix]
            rhs_arr_ix     = (rhs_a[i_r][i_r] + shiftr[ix] * darrdx_advec_R[ix] 
                               + 2.0 * arr[ix] * dshiftrdx[ix])
            rhs_att_ix     = (rhs_a[i_t][i_t] + shiftr[ix] * dattdx_advec_R[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * a[i_t][i_t]) # additional advection terms from rescaling
            rhs_app_ix     = (rhs_a[i_p][i_p] + shiftr[ix] * dappdx_advec_R[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * a[i_p][i_p]) # additional advection terms from rescaling
            rhs_lambdar_ix += (shiftr[ix] * dlambdardx_advec_R[ix] 
            ...
            rhs_u_ix       += shiftr[ix] * dudx_advec_L[ix]
            rhs_v_ix       += shiftr[ix] * dvdx_advec_L[ix]
            rhs_phi_ix     += shiftr[ix] * dphidx_advec_L[ix]
            rhs_hrr_ix     = (rhs_h[i_r][i_r] + shiftr[ix] * dhrrdx_advec_L[ix]
                               + 2.0 * hrr[ix] * dshiftrdx[ix])
            rhs_htt_ix     = (rhs_h[i_t][i_t] + shiftr[ix] * dhttdx_advec_L[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * h[i_t][i_t]) # additional advection terms from rescaling
            rhs_hpp_ix     = (rhs_h[i_p][i_p] + shiftr[ix] * dhppdx_advec_L[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * h[i_p][i_p]) # additional advection terms from rescaling
            rhs_K_ix       += shiftr[ix] * dKdx_advec_L[ix]
            rhs_arr_ix     = (rhs_a[i_r][i_r] + shiftr[ix] * darrdx_advec_L[ix] 
                               + 2.0 * arr[ix] * dshiftrdx[ix])
            rhs_att_ix     = (rhs_a[i_t][i_t] + shiftr[ix] * dattdx_advec_L[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * a[i_t][i_t]) # additional advection terms from rescaling
            rhs_app_ix     = (rhs_a[i_p][i_p] + shiftr[ix] * dappdx_advec_L[ix]
                               + 2.0 * shiftr[ix] * 1.0/r_here * a[i_p][i_p]) # additional advection terms from rescaling
            rhs_lambdar_ix += (shiftr[ix] * dlambdardx_advec_L[ix] 
                                - lambdar[ix] * dshiftrdx[ix])
            # NB optional to add advection to lapse and shift vars
    ...
    sigma = 10.0 # kreiss-oliger damping coefficient, max_step should be limited to 0.1 R/N_ri
