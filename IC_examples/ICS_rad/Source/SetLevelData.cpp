                            ...
                            const PoissonParameters &a_params)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            // set rho according to user defined function
            multigrid_vars_box(iv, c_rho_0) =
                my_rho_function(loc, a_params.phi_background, a_params.phi_amplitude,
             ...
             const PoissonParameters &a_params, const Real constant_K)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        //                    CHF_CONST_FRA1(multigrid_vars_box, c_rho_0),
        //                    CHF_CONST_REAL(a_dx[0]), CHF_BOX(this_box));
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            Real m = 0;
            set_m_value(m, multigrid_vars_box(iv, c_rho_0), a_params,
                        constant_K);
            ...
            rhs_box(iv, 0) =
                ...
                // 2.0 * M_PI * a_params.G_Newton * rho_0 -
                              ...
                              const PoissonParameters &a_params)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            Real m = 0;
            set_m_value(m, multigrid_vars_box(iv, c_rho_0), a_params, 0.0);
...
void set_m_value(Real &m, const Real &rho_here,
                 const PoissonParameters &a_params, const Real constant_K)
{
    ...
    // Real scalar_mass = 2.5e-12;    // CJ

    // V_of_phi =  scalar_mass * pow(1.0*Mp, 4.0) *
    //     pow(1.0 - exp(-sqrt(2.0/3.0) * abs(rho_here) / Mp), 2.0); // CJ
                     ...
                     const Real &constant_K)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            // TODO: Set fluid vars
            grchombo_vars_box(iv, c_E) = multigrid_vars_box(iv, c_rho_0);  
            grchombo_vars_box(iv, c_D) = (multigrid_vars_box(iv, c_rho_0) *0 + 1e-18);    
            grchombo_vars_box(iv, c_pressure) = (multigrid_vars_box(iv, c_rho_0) * 0.3333333333 );  
            grchombo_vars_box(iv, c_W) = (multigrid_vars_box(iv, c_rho_0) *0 + 1); 
