    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            rhs_box(iv, 0) =
                ...
                2.0 * M_PI * a_params.G_Newton * 0.5*(rho_gradient(iv, 0) + rho_gradient2(iv, 0)) * psi_0 -
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            integrand_box(iv, 0) =
                ...
                24.0 * M_PI * a_params.G_Newton * 0.5*(rho_gradient(iv, 0) + rho_gradient2(iv, 0))*
                    pow(psi_0, -4.0) +
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            condition_box(iv, 0) = 1.5 * abs(m) + 1.5 * A2 * pow(psi_0, -7.0) +
                                   24.0 * M_PI * a_params.G_Newton *
                                       abs( 0.5*(rho_gradient(iv, 0) + rho_gradient2(iv, 0))) *
                                       pow(psi_0, 1.0) +
                                   log(psi_0);
        }
    }
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            aCoef_box(iv, 0) =
                ...
                2.0 * M_PI * a_params.G_Newton * 0.5*(rho_gradient(iv, 0) + rho_gradient2(iv, 0));
        }
    }
}
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            multigrid_vars_box(iv, c_rho_0) =
                ...
                0.5*(rho_gradient(iv, 0) + rho_gradient2(iv, 0)) * pow(psi_0, -4.0);
        }
    }
} // end set_rho
