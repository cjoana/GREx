    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            multigrid_vars_box(iv, c_phi2_0) =
                my_phi2_function(loc, a_params.phi_background, a_params.phi_amplitude,
                                a_params.phi_wavelength, a_params.domainLength);
             ...
             const PoissonParameters &a_params, const Real constant_K)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            //TODO: Set G00, G11 and add it into rho_(2). 
            Real sf = multigrid_vars_box(iv, c_phi_0);    // phi_0
            Real sf2 = multigrid_vars_box(iv, c_phi2_0);  // phi2_0
            Real f_coeff = 0.5 *(1 + a_params.sf1_a_coupling * sf*sf 
                                     + a_params.sf2_a_coupling * sf2*sf2 );
            Real G_coeff = 1.0 / (4*f_coeff*f_coeff);
            Real G00 = G_coeff *(2*f_coeff + 6*a_params.sf1_a_coupling*a_params.sf1_a_coupling*sf*sf);
            Real G11 = G_coeff *(2*f_coeff + 6*a_params.sf2_a_coupling*a_params.sf2_a_coupling*sf2*sf2);
            
            

            rhs_box(iv, 0) =
                0.125 * m * pow(psi_0, 5.0) - 0.125 * A2 * pow(psi_0, -7.0) -
                2.0 * M_PI * a_params.G_Newton * (G00 * rho_gradient(iv, 0) + G11* rho_gradient2(iv, 0)) * psi_0 -
                              ...
                              const PoissonParameters &a_params)
{
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            //TODO: Set G00, G11 and add it into rho_gradient(2). 
            Real sf = multigrid_vars_box(iv, c_phi_0);    // phi_0
            Real sf2 = multigrid_vars_box(iv, c_phi2_0);  // phi2_0
            Real f_coeff = 0.5 *(1 + a_params.sf1_a_coupling * sf*sf 
                                     + a_params.sf2_a_coupling * sf2*sf2 );
            ...
            integrand_box(iv, 0) =
                ...
                24.0 * M_PI * a_params.G_Newton * (G00 * rho_gradient(iv, 0) + G11* rho_gradient2(iv, 0))*
                    pow(psi_0, -4.0) +
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            condition_box(iv, 0) = 1.5 * abs(m) + 1.5 * A2 * pow(psi_0, -7.0) +
                                   24.0 * M_PI * a_params.G_Newton *
                                       abs( (G00 * rho_gradient(iv, 0) + G11* rho_gradient2(iv, 0))  ) *
                                       pow(psi_0, 1.0) +
                                   log(psi_0);
        }
    }
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            aCoef_box(iv, 0) =
                ...
                2.0 * M_PI * a_params.G_Newton * (G00 * rho_gradient(iv, 0) + G11* rho_gradient2(iv, 0));
        }
    }
}
    ...
    for (dit.begin(); dit.ok(); ++dit)
    {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
            ...
            multigrid_vars_box(iv, c_rho_0) =
                ...
                (G00 * rho_gradient(iv, 0) + G11* rho_gradient2(iv, 0)) * pow(psi_0, -4.0);
        }
    }
} // end set_rho
