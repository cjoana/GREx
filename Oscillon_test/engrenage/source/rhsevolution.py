...
def get_rhs(t_i, current_state, R, N_r, r_is_logarithmic, eta, progress_bar, time_state) :
    ...
    rho_arr =  np.zeros_like(lapse)
    S_arr = np.zeros_like(lapse)
    ...
    for ix in range(num_ghosts, N-num_ghosts) :
        ...
        rhs_K[ix]       = get_rhs_K(r_here, lapse[ix], K[ix], Asquared, em4phi, d2lapsedx2[ix], dlapsedx[ix], 
    ...
    if DEBUG:
        ...
        print(f"u = {u[idr]}  "+  f"du = {rhs_u[idr]}"  )
        print(f"v = {v[idr]}  "+  f"dv = {rhs_v[idr]}" )
        print(f"chi = {phi[idr]}  "+  f"dchi = {rhs_phi[idr]}" )

        print(f"K = {K[idr]}  "+  f"da = {rhs_K[idr]}")

        print(f"hrr = {hrr[idr]}  "+  f"dhrr = {rhs_hrr[idr]}")
        print(f"htt = {htt[idr]}  "+   f"dhtt = {rhs_htt[idr]}" )


        print(f"arr = {arr[idr]}  "+      f"dArr = {rhs_arr[idr]}" )

        print(f"rho, S = {rho_arr[idr]},  {S_arr[idr]} , {S_arr[idr]/rho_arr[idr]/3} ")
