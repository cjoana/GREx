...
def get_rhs(t_i, current_state, params, sigma, progress_bar, time_state) :     
    ...
    matter_Sa, matter_Sb   = get_Sa_Sb(phi[mask], psy[mask], Pi[mask], a[mask], em4chi[mask])
    ...
    if(r_is_logarithmic) : #take logarithmic derivatives
        ...
        dr_lapsePi = get_dfdx(lapse*Pi, oneoverdx)

        ## DEBUG, set fancy derivs to Zero
        # cov_beta = get_covbeta(r, a, b, dadr, dbdr, beta, dbetadr) *0
        # dr_cov_beta = get_dfdx(cov_beta, oneoverdx)  *0
        # dr_beta_over_r = get_dfdx(beta/r, oneoverdx) *0
        # dr_dlapsedr_over_r = get_dfdx(dlapsedr/r, oneoverdx) *0
        # dr_dchidr_over_r = get_dfdx(dchidr/r, oneoverdx) *0
        # dr_Lambda_over_r = get_dfdx(Lambda/r, oneoverdx) *0
        # dr_lapsePi = get_dfdx(lapse*Pi, oneoverdx) *0
    ...
    # print("  ", t_i, last_t, deltat, time_state)
    ...
    if DEBUG:
        idr = num_ghosts +1
        print(f"Debug: at r = {r[idr]}, t_i= {t_i} ")

        print(f"phi = {phi[idr]}  "+  f"dphi = {rhs_phi[idr]}"  )
        print(f"Pi = {Pi[idr]}  "+  f"dPi = {rhs_Pi[idr]}" )
